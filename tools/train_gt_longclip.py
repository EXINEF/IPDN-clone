...
from ipdn.dataset import build_dataloader, build_dataset_longclip
from ipdn.model.ipdn_gt_longclip import IPDN
    ...
    for batch in dataloader:
        ...
        if gorilla.is_main_process():
            ...
            if 'meta_datas' in result.keys():
                ...
                #view_dependents.extend(result['view_dependents'])
            #else:
            #    print('No meta_datas')
            progress_bar.update()
...
def main(args):
    ...
    if not args.skip_training:
        train_dataset = build_dataset_longclip(cfg.data.train, logger)
    ...
    if not args.skip_validate and gorilla.is_main_process():
        val_dataset = build_dataset_longclip(cfg.data.val, logger)
    ...
    # model.calculate_pos_weight(train_dataset, val_dataset)
