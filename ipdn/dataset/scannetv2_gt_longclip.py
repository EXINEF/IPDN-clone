...
class ScanNetDataset_sample_graph_edge(Dataset):
    def __init__(self,
        ...
        if self.dataset == 'scanrefer':
            ...
            if self.prefix == 'train':
                self.scanrefer = json.load(open(os.path.join(self.data_root, 'ScanRefer', 'ScanRefer_train_new.json')))
                if is_main_process(): self.logger.info(f'Load {self.prefix} scanrefer: {len(self.scanrefer)} samples')
            elif self.prefix == 'val':
                self.scanrefer = json.load(open(os.path.join(self.data_root, 'ScanRefer', 'ScanRefer_val_new.json')))
                if is_main_process(): self.logger.info(f'Load {self.prefix} scanrefer: {len(self.scanrefer)} samples')
            else:
    ...
    def transform_train(self, xyz, rgb, superpoint, semantic_label, instance_label):
        ...
        unique, superpoint = np.unique(superpoint[valid_idxs], return_inverse=True)
        # instance_label = self.get_cropped_inst_label(instance_label, valid_idxs)
        instance_label = instance_label[valid_idxs]
        return xyz, xyz_middle, rgb, superpoint, semantic_label, instance_label, unique
    ...
    def __getitem__(self, index: int) -> Tuple:
        ann_ids, object_idss, gt_pmasks, gt_spmasks, sp_ref_masks, lang_utterances, dense_maps, pairss = [], [], [], [], [], [], [], []
        ...
        for i in range(len(scene_input)):
            ...
            if i==0:
                ...
                xyz, xyz_middle, rgb, superpoint, semantic_label, instance_label, unique = scene
                ...
                sp_ins_label, _ = self.get_sp_label(superpoint, instance_label, semantic_label)
                
                feat_2d = torch.load('data/clip-feat/'+scan_id+'.pth')[torch.tensor(unique).long()]
                #feat_2d = torch.load('data/i2sp/'+scan_id+'.pth')[torch.tensor(unique).long()]#nr3d sr3d
    ...
    def collate_fn(self, batch: Sequence[Sequence]) -> Dict:
        ...
        # else:
        #     raise NotImplementedError(f'Text encoder {TEXT_ENCODER} not supported')
        token_dict = tokenized_long_clip_style_with_standard_clip(self.tokenizer, lang_utterances)
        ...
        feats_2d = torch.cat(feats_2d,dim=0)
    ...
    def _get_token_positive_map_by_parse(self, anno, auxi_box):
        ...
        # else:
        #     raise NotImplementedError(f'Text encoder {TEXT_ENCODER} not supported')
        tokenized = tokenized_long_clip_style_with_standard_clip(self.tokenizer, [' '.join(anno['utterance'].replace(',', ' ,').split())])
        ...
        gt_map_t = get_positive_map(tokenized, target_char_span[:num_t]) # [短语数量，句子max长度] 值为span内所有位置平分1的概率，例如[[0.5, 0.5, 0, 0, 0]]
        ...
        gt_map_t = gt_map_t.sum(axis=0) # 将所有短语的span合并，得到句子长度的概率向量
        ...
        target_positive_map[:1] = gt_map_t          # main object 因为rec目标只有1个，所以就把第0个搞成gt
...
def tokenized_long_clip_style_with_standard_clip(tokenizer, text):
    ...
    for i in range(input_ids.shape[0]):
        eos_positions = (input_ids[i] == tokenizer.eos_token_id).nonzero(as_tuple=True)[0]
        if len(eos_positions) > 0:
            pad_start = eos_positions[0] + 1
