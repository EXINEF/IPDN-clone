...
class DEC(nn.Module):
    ...
    def forward_iter_pred(self, x, fps_seed_sp, batch_offsets, lang_feats=None, lang_masks=None, sp_pos=None, feats_2d=None, scene_object_embeds=None, scenes_len=None):
        ...
        # # IMPORTANT before returning the mask, use this self.out_norm = nn.LayerNorm(d_model)
        # # Instead of just min/max, also check:
        # print(f"DEBUG BEFORE: min={query.min():.4f}, max={query.max():.4f}, mean={query.mean():.4f}, std={query.std():.4f}")
        # query = self.out_norm(query[:,:128])
        # print(f"DEBUG AFTER:  min={query.min():.4f}, max={query.max():.4f}, mean={query.mean():.4f}, std={query.std():.4f}")
        # exit()
        # print(f"DEBUG: projection queries length: {len(proj_queries)}")

        # print(f"\n=== ALL PROJ_QUERIES ANALYSIS ===")
        # for i, proj_q in enumerate(proj_queries):
        #     if proj_q is not None:
        #         print(f"PROJ_QUERIES[{i}] ({'initial' if i==0 else f'layer_{i}'}):")
        #         print(f"  Shape: {proj_q.shape}")
        #         print(f"  Min: {proj_q.min():.4f}, Max: {proj_q.max():.4f}")
        #         print(f"  Mean: {proj_q.mean():.4f}, Std: {proj_q.std():.4f}")
        #     else:
        #         print(f"PROJ_QUERIES[{i}]: None")
        #     print()

        # print(f"RAW QUERY (final):")
        # print(f"  Shape: {query[:,:128].shape}")
        # print(f"  Min: {query[:,:128].min():.4f}, Max: {query[:,:128].max():.4f}")
        # print(f"  Mean: {query[:,:128].mean():.4f}, Std: {query[:,:128].std():.4f}")
        # print(f"====================================\n")
        # exit()
        # exit()
